...
export interface ExpirationPlugin extends StrategyPlugin {
 ...
 */
export const expiration = (config: ExpirationPluginOptions = {}) => {
  ...
  let cacheExpirations = new Map<string, CacheExpiration>();

  if (!config.maxAgeFrom) {
    config.maxAgeFrom = "last-fetched";
  }
   ...
   */
  const deleteCacheAndMetadata = async (): Promise<void> => {
    // Do this one at a time instead of all at once via `Promise.all()` to
    // reduce the chance of inconsistency if a promise rejects.
    for (const [cacheName, cacheExpiration] of cacheExpirations) {
      await self.caches.delete(cacheName);
      await cacheExpiration.delete();
    }

    // Reset this._cacheExpirations to its initial state.
    cacheExpirations = new Map();
  };

  if (config.purgeOnQuotaError) {
    registerQuotaErrorCallback(() => deleteCacheAndMetadata());
  }
   ...
   * @private
   */
  const getCacheExpiration = (cacheName: string): CacheExpiration => {
    if (cacheName === privateCacheNames.getRuntimeName()) {
      throw new SerwistError("expire-custom-caches-only");
    }

    let cacheExpiration = cacheExpirations.get(cacheName);
    if (!cacheExpiration) {
      cacheExpiration = new CacheExpiration(cacheName, config);
      cacheExpirations.set(cacheName, cacheExpiration);
    }
   ...
   * @private
   */
  const getDateHeaderTimestamp = (cachedResponse: Response): number | null => {
  ...
  const isResponseDateFresh = (cachedResponse: Response): boolean => {
    const isMaxAgeFromLastUsed = config.maxAgeFrom === "last-used";
    ...
    // discussion.
    const dateHeaderTimestamp = getDateHeaderTimestamp(cachedResponse);
    ...
    return dateHeaderTimestamp >= now - config.maxAgeSeconds * 1000;
  };
  ...
  return {
    ...
    cachedResponseWillBeUsed({ event, cacheName, request, cachedResponse }: CachedResponseWillBeUsedCallbackParam) {
      ...
      const isFresh = isResponseDateFresh(cachedResponse);

      // Expire entries to ensure that even if the expiration date has
      // expired, it'll only be used once.
      const cacheExpiration = getCacheExpiration(cacheName);
