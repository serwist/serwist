<script>
  import ICD from "$components/InlineCode.svelte";
</script>

<h1 id="configuring">Configuring</h1>
<h2 id="available-options">Available options</h2>
<p>There are various options you can use to customize the behaviour of the plugin:</p>
<ul class="list">
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/additional-precache-entries">additionalPrecacheEntries</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/base">base</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/dev-options">devOptions</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/disable">disable</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/dont-cache-bust-urls-matching">dontCacheBustURLsMatching</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/glob-directory">globDirectory</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/glob-follow">globFollow</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/glob-ignores">globIgnores</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/glob-patterns">globPatterns</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/glob-strict">globStrict</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/injection-point">injectionPoint</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/integration">integration</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/manifest-transforms">manifestTransforms</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/maximum-file-size-to-cache-in-bytes">maximumFileSizeToCacheInBytes</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/mode">mode</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/modify-url-prefix">modifyURLPrefix</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/plugins">plugins</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/rollup-format">rollupFormat</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/rollup-options">rollupOptions</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/scope">scope</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/sw-dest">swDest</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/sw-src">swSrc</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/sw-url">swUrl</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/templated-urls">templatedURLs</a>
    </ICD>
  </li>
  <li>
    <ICD>
      <a class="link" href="/docs/vite/configuring/type">type</a>
    </ICD>
  </li>
</ul>
