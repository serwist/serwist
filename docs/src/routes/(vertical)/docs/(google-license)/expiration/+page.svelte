...
<br />
<p>
  <ICD>@serwist/expiration</ICD> is a module that expires cached responses based
  on age or maximum number of entries.
</p>
<br />
<p>
  ...
  in a cache. Serwist provides this functionality through this module, which
  allows you to limit the number of entries in a cache and/or remove entries
  that have been cached for a long period of time.
</p>
<br /><br />
<h2 id="basic-usage">Basic usage</h2>
<br />
<h3 id="restrict-the-number-of-cache-entries">
...
<br />
<p>
  After a cached response is used or a new request is added to the cache, the
  plugin will look at the configured cache and ensure that the number of cached
  entries doesn't exceed the limit. If it does, the oldest entries will be
  removed.
</p>
<br />
<h3 id="restrict-the-age-of-cached-entries">
  Restrict the age of cached entries
</h3>
<br />
<p>
  To restrict how long a request is cached for, you can define a max age in
  seconds using the <ICD>maxAgeSeconds</ICD> option like so:
</p>
<br />
...
<br />
<p>
  If you'd like to use the expiration logic separate from any other Serwist
  module, you can do so with the <ICD>CacheExpiration</ICD> class.
</p>
<br />
<p>
